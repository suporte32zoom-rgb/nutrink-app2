...
interface PrescriptionsGlobalViewProps {
  patients: Patient[];
...
export const PrescriptionsGlobalView: React.FC<PrescriptionsGlobalViewProps> = ({
  patients,
  onUpdatePatient,
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-black text-white flex items-center gap-2">
                Central de Prescrições & Suplementação
              </h1>
              <p className="text-xs sm:text-sm text-purple-200/80">
                Fitoterápicos, nutracêuticos, manipulados, dosagens e orientações posológicas.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#140428] border border-purple-800/60 rounded-2xl p-4 space-y-3 shadow-md">
            ...
            <div className="max-h-[500px] overflow-y-auto space-y-2 pr-1">
              {filteredPatients.map((p) => {
                ...
                const prescCount = p.prescriptions ? p.prescriptions.length : 0;
                return (
                  <div
                    key={p.id}
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-purple-300/80 mt-0.5">{p.objective.replace('_', ' ')}</div>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-purple-950 text-fuchsia-300 border border-purple-700/50">
                        {prescCount} {prescCount === 1 ? 'Receita' : 'Receitas'}
                      </span>
        ...
        <div className="lg:col-span-8">
          {selectedPatient ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="text-sm sm:text-base font-bold text-white">
                      Prescrições de {selectedPatient.name}
                    </h2>
                    <p className="text-xs text-purple-300">
                      Fórmulas magistrais, suplementos e protocolos ativos
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-[#140428] border border-purple-800/60 rounded-3xl p-12 text-center text-purple-300">
              <Pill className="w-12 h-12 mx-auto text-purple-500/50 mb-3" />
