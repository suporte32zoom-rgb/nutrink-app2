...
- **NutriCalc Avançado**: Cálculos metabólicos de Harris-Benedict, Cunningham e Mifflin-St Jeor.`,
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-fuchsia-500 selection:text-white">
      ...
      {/* Floating NÚTRIA Action Button (Desktop: visible at bottom-8 right-8, when not on nutria_hub and drawer is closed) */}
      {currentTab !== 'nutria_hub' && !isFloatingChatOpen && (
        <button
          onClick={() => setIsFloatingChatOpen(true)}
          className="hidden lg:flex fixed lg:bottom-8 lg:right-8 z-50 px-5 py-3 bg-gradient-to-r from-fuchsia-600 via-purple-600 to-indigo-600 hover:from-fuchsia-500 hover:to-purple-500 text-white rounded-full shadow-2xl shadow-purple-950/90 hover:shadow-fuchsia-500/50 items-center justify-center gap-2.5 font-bold text-sm transition-all duration-200 hover:scale-105 active:scale-95 border border-fuchsia-400/60 group cursor-pointer opacity-100 visible"
          id="btn-open-nutria-floating"
          title="Falar com Copiloto NÚTRIA IA"
        >
          <div className="relative shrink-0 flex items-center justify-center">
            <Bot className="w-5 h-5 text-white" />
            <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-fuchsia-300 rounded-full animate-ping"></span>
          </div>
          <span className="text-sm font-bold tracking-tight text-white whitespace-nowrap opacity-100">
            Falar com NÚTRIA
          </span>
        </button>
      )}
      ...
      {isFloatingChatOpen && (
        <div 
          className="fixed bottom-20 sm:bottom-24 lg:bottom-8 right-2.5 sm:right-4 lg:right-8 z-50 w-[calc(100vw-20px)] sm:w-full max-w-lg p-0.5 sm:p-2 max-h-[80vh] lg:max-h-[85vh] flex flex-col box-border min-w-0"
        >
          <NutriaCopilot
            messages={nutriaMessages}
            onSendMessage={handleSendNutriaMessage}
